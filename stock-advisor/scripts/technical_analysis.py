...
def format_report(result: dict, symbol: str, name: str = "") -> str:
    """格式化技术分析报告为文本"""
    lines = []
    title = f"{name}({symbol})" if name else symbol
    lines.append(f"=== {title} 技术分析 ===")
    lines.append(f"当前价格: {result['price']['current']} ({result['price']['change_pct']:+.2f}%)")
    lines.append(f"趋势判断: {result['trend']}")
    lines.append(f"支撑位: {result['support_resistance']['support']} | 阻力位: {result['support_resistance']['resistance']}")
    lines.append(f"\n技术指标:")
    for ind_name, data in result['indicators'].items():
        if isinstance(data, dict):
            signal = data.get('signal', '')
            lines.append(f"  {ind_name}: {signal}")
        else:
            lines.append(f"  {ind_name}: {data}")
    score = result['score']
    lines.append(f"\n综合评分: {score['score']} - {score['rating']} ({'⭐' * score['stars']})")
    return "\n".join(lines)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--days", type=int, default=120, help="获取天数（默认120）")
    ...
    if args.json:
        ...
        for ind_name, data in result['indicators'].items():
            if isinstance(data, dict):
                json_result['indicators'][ind_name] = {
                    k: (round(v, 4) if isinstance(v, float) else v)
                    for k, v in data.items()
                }
            else:
                json_result['indicators'][ind_name] = data
        print(json.dumps(json_result, ensure_ascii=False, indent=2))
    else:
        print(format_report(result, args.symbol))
